    ...
    stage('Tests') {
      steps {
        parallel(
          "Plone6 & Python3": {
            node(label: 'docker') {
              sh '''docker run --pull="always" -i --name="$BUILD_TAG-tests" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e DEVELOP="src/$GIT_NAME src/eea.kitkat" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/plone-test:6'''
